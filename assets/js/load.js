...
function monthDiff(dateFrom, dateTo) {
	const numMonth = dateTo.getMonth() - dateFrom.getMonth() +
	...
	fetch("./assets/data/experience.json")
		...
		.then((json) => {
			...
			json.forEach(ele => {
				...
				let parseDateEnd = ele.Date.End ? new Date(ele.Date.End) : new Date()
				let parseMonthEnd = parseDateEnd.getMonthNameShort('en')
				let parseYearEnd = parseDateEnd.getFullYear()
				let parseDateEndString = ele.Date.End ? `${parseMonthEnd} ${parseYearEnd}` : 'Present'
